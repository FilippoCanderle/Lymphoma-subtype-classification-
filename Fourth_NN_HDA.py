...
def Compile_Train_cnn_4(cnn,X_train, Y_train,X_test, Y_test):
    batch_size=128
    nb_epochs=6
...
def Model_history_4(My_keras_model):
...
def Model_save_4(cnn):
    cnn.save("Lymphoma_CNN_Classifier_fourth_model.h5")
    
def Load_Model_4():
    from keras.models import load_model
    new_model = load_model("Lymphoma_CNN_Classifier_fourth_model.h5")
    
    return new_model
    
def Model_evaluation_4(X_test, Y_test):
    score = Load_Model_2().evaluate(X_test, Y_test, verbose = 0) 

    print('Test loss:', score[0]) 
    print('Test accuracy:', score[1])
    
def Image_prediction_4(path="tests_predictions/cll.tif"):
